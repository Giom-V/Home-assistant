...
def encodeXML(values:list[list[int, any]]) -> str:
    xml = "<valeurs>"
    for value in values:
        xml += "<valeur donnee=\"" + value[0].ToString() + "\" valeur=\"" + base64.b64encode(value[1].ToString()) + "\" />"
    xml += "</valeurs>"
...
class Solisart:
    ...
    async def log_in_to_get_cookie(self) -> bool:
        ...
        response = await self._hass.async_add_executor_job(lambda : session.request("POST", URL, headers=headers, data=payload, files=files))
    ...
    async def fetch_data(self) -> dict[str, any]:
        ...
        await self.log_in_to_get_cookie() # TODO: Only relog in when needed (fetching fails)
        ...
        session = self._session
        response = await self._hass.async_add_executor_job(lambda : session.request("POST", URL_DATA, headers=headers, data=payload, files=files))
        ...
        for value in xmlRoot.iter("valeur"):
            ...
            data[int(value.get("donnee"))] = base64.b64decode(value.get("valeur")).decode("utf-8")
