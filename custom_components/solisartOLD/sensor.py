...
from .solisart import decodeValue
...
async def async_setup_platform(
    ...
    for temp_sensor in TEMPERATURE_SENSORS:
        temp_sensors.append(
            SolisartTemperatureSensor(coordinator, temp_sensor[0], temp_sensor[1])
        )
    ...
    for pump_sensor in PUMP_SENSORS:
        pump_sensors.append(
            SolisartPumpSensor(coordinator, pump_sensor[0], pump_sensor[1])
        )
...
class SolisartTemperatureSensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(
        self, coordinator: DataUpdateCoordinator, name: str, value_id: int
    ) -> None:
    ...
    def state(self) -> float:
        ...
        return decodeValue(self.coordinator.data[self._value_id])
...
class SolisartSolarEfficiencySensor(CoordinatorEntity, SensorEntity):
    ...
    def state(self) -> float:
        ...
        return (
            float(decodeValue(self.coordinator.data[C5_WATER_SUN]))
            + (1 - float(decodeValue(self.coordinator.data[V3V_HEAT_PUMP])) / 100)
            * (
                float(decodeValue(self.coordinator.data[C7_UNDERFLOOR]))
                + float(decodeValue(self.coordinator.data[C2_FIRST_FLOOR]))
                + float(decodeValue(self.coordinator.data[C3_GROUND_FLOOR]))
                + float(decodeValue(self.coordinator.data[C6_SPA]))
            )
        ) * (
            float(decodeValue(self.coordinator.data[T1_PANELS_OUT]))
            - float(decodeValue(self.coordinator.data[T2_PANELS_IN]))
        )
