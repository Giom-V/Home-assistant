...
[automations.yaml](../automations.yaml) file iw that way:
...
-   Maybe having different upper values for the temperature depending on the
    seasons
-   I should group all temperature sensors in a group so that I won't have to
    list them all in each automation
-   Leak detection
...
-   Find a way to be able to tell the vacuum not to vaccum right now but to do
    it later
-   Use "And/Or" instead of multiple ifs to make the `Update xiaomi map
    extractor` automation easier to read
...
-   `Sets that I'm working when I'm at home`: Says I'm working at the begining
    of my shift if I'm at home.
...
-   `Sets that I stop working at the end of my shift`: End my working time at
    the time entered in `input_datetime.work_end`.
...
-   Good night routine which switches off lights, close curtains, and tell me if
    a door is open
-   good morning routine, with also a summary of my day (do I have a lunch
    ...
    etc...)
-   Detect if a car approches the house (I want to be able to say "I'm waiting
    for someone, tell me when they arrive")
-   Circadian lights
